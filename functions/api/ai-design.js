...
- Otherwise, choose colors deliberately for the brand's actual style and vibe, not just its industry category. Identify the personality implied by the brief and any brand guide tone/notes provided — e.g. luxury/premium (deep, muted, low-saturation tones: black, charcoal, gold, deep jewel tones — avoid bright/candy colors), playful/youthful (high-saturation, energetic hues), trustworthy/corporate (blues, teals, restrained saturation), organic/wellness (muted greens, earth tones, soft neutrals), bold/disruptive (high contrast, unexpected pairings) — then pick specific hues, saturation, and lightness that actually embody that vibe, not a default/placeholder palette.
- "text" and "subtext" must contrast clearly against "bg" (don't pick near-identical colors).
- Do not default to a blue+yellow/gold pairing (in either direction — blue bg with yellow accent, or yellow/gold bg with navy accent/text) regardless of brief. That specific pairing keeps recurring as a generic fallback; only use it when it's actually the right fit (e.g. it's the named brand's real colors). Vary hue, saturation, and which color plays bg vs accent based on what this specific brief actually calls for — most briefs should NOT land on blue+yellow.`
